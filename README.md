...
- 🎯 Entry points to execute your program using `python -m <zuma>` or `$ zuma` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/demoxie/zuma/branch/main/graph/badge.svg?token=zuma_token_here)](https://codecov.io/gh/demoxie/zuma)
[![CI](https://github.com/demoxie/zuma/actions/workflows/main.yml/badge.svg)](https://github.com/demoxie/zuma/actions/workflows/main.yml)

Awesome zuma created by demoxie

## Install

from source
```bash
git clone https://github.com/demoxie/zuma zuma
cd zuma
make install
```
...
```bash
❯ zuma --help
Usage: zuma [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ zuma create-user --help
Usage: zuma create-user [OPTIONS] USERNAME PASSWORD
...
```bash
zuma create-user admin admin --superuser
```
...
Run with `zuma run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `zuma create-user`.
...
-----------------------------------------------------
zuma/__init__.py              4      0   100%
zuma/app.py                  16      1    94%
zuma/cli.py                  21      0   100%
zuma/config.py                5      0   100%
zuma/db.py                   10      0   100%
zuma/models/__init__.py       0      0   100%
zuma/models/content.py       47      1    98%
zuma/routes/__init__.py      11      0   100%
zuma/routes/content.py       52     25    52%
zuma/routes/security.py      15      1    93%
zuma/routes/user.py          52     26    50%
zuma/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from zuma.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in zuma/default.toml.

### As environment variables
```bash
export zuma_KEY=value
export zuma_KEY="@int 42"
export zuma_KEY="@jinja {{ this.db.uri }}"
export zuma_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
